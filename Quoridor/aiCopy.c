/*
 * aiCopy.c a copy of the original ai code
 *
...
void minpathsCpy(int size, char **walltrack, int *minpathw, int *minpathb) {
...
void getmoveCpy(int size, char** walltrack, char* color, int move, int *wins, int *sims, int *chosenr, int *chosenc, char *chosenori, int analdepth, int* foundmove, double startvaluew, double startvalueb) {
	...
	//printf(" AImove = %d",move);//TODO: kill later.
	...
	if (move==2) {//check for winning move immidiately
		if (strcmp(color, "white")==0) {//if white in blackrow
			if (black.row==0) {
				*wins=1;
				*sims=1;
				return;
